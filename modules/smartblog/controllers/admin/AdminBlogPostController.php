...
//             $this->updateAccessories($object);
   ...
   protected function postImage($id) {
		...
		if (count($post_types) == 0) {
		    $post_types = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                                    SELECT * FROM `' . _DB_PREFIX_ . 'smart_blog_imagetype` where `active` = 1 and `type` = "post"');
		}


//		echo "<pre>";print_r($post_types);die('</pre>');
...
//                ),
    ...
    public function deleteImage($id_smart_blog_post = 1) {
        ...
        foreach (Language::getLanguages(false) as $lang) {
            $files_to_delete[] = _PS_TMP_IMG_DIR_ . 'smart_blog_post_mini_' . $id_smart_blog_post .'_'. (int)$lang['id_lang'] .'.jpg';
        }
