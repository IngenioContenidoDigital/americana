...
{else}
    ...
    {if isset($ispost) && !empty($ispost)}
        <a itemprop="url" style="background-image: url('/modules/smartblog/images/{$post.id_post}-single-default.jpg');" href="{$smartbloglink->getSmartBlogPostLink($post.id_post,$post.link_rewrite)|escape:'htmlall':'UTF-8'}" title="{$post.meta_title|escape:'htmlall':'UTF-8'}" class="imageFeaturedLink">    
        ...
        {if isset($cameFromLoop)} 
            ...
            {else} 
                {if ($post.post_img != "no" && $activeimgincat == 0) || $activeimgincat == 1} 
                    <img itemprop="image" alt="{$post.meta_title|escape:'htmlall':'UTF-8'}" src="/modules/smartblog/images/{$post.id_post}-single-default.jpg" class="imageFeatured">
