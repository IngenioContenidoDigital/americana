...
class PayulatamConfirmModuleFrontController extends ModuleFrontController{
    ...
    public function initContent(){
        ...
        if (isset($_REQUEST['sign'])) $signature = $_REQUEST['sign']; else $signature = $_REQUEST['firma'];
        
        if (isset($_REQUEST['merchant_id'])) $merchant_id = $_REQUEST['merchant_id']; else $merchant_id = $_REQUEST['usuario_id'];
        
        if (isset($_REQUEST['reference_sale'])) $reference_code = $_REQUEST['reference_sale']; else $reference_code = $_REQUEST['ref_venta'];
        
        if (isset($_REQUEST['value'])) $value = $_REQUEST['value']; else $value = $_REQUEST['valor']; 
        
        if (isset($_REQUEST['currency'])) $currency = $_REQUEST['currency']; else $currency = $_REQUEST['moneda'];
        
        if (isset($_REQUEST['state_pol'])) $transaction_state = $_REQUEST['state_pol']; else $transaction_state = $_REQUEST['estado_pol'];

        $cart_id = explode('_', $reference_code);
        $split = explode('.', $value);
        $decimals = $split[1];
        
        if ($decimals % 10 == 0) $value = number_format($value, 1, '.', '');
        ...
        if (isset($_REQUEST['response_code_pol'])) $pol_response_code = $_REQUEST['response_code_pol']; else $pol_response_code = $_REQUEST['codigo_respuesta_pol'];
	...
	if (!Validate::isLoadedObject($cart)) $errors[] = $this->module->l('Invalid Cart ID');
	else {
