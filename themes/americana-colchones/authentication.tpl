...
{else}
    ...
    {/if}-->
        <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box">
		    ...
		    <div class="col-md-4">
                        <div class="required row form-group">
                            <label for="id_dni_type">{l s='Tipo de Identificación'}</label>
                            <select name="id_dni_type" id="id_dni_type" class="form-control">
                                    <option value="0" selected="selected">{l s='Elija Tipo de Identificación'}</option>
                                {foreach from=$id_types key=k item=type}
                                    <option value="{$type.id_dni_type}">{$type.dni_type}</option>
                                {/foreach}
                            </select>
                        </div>
                        <div class="required row form-group">
                            <label for="dni">{l s='Número de Identificación'} <sup>*</sup></label>
                            <input type="text" class="is_required form-control" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni}{/if}" />
                        </div>
		    </div>
                    <div class="col-md-4">
                        <div class="form-group">
            ...
            {if $b2b_enable}
                <div class="account_creation" id="company-info">
        ...
        {/if}
            {literal}
                <script>
                    $('input[name=id_gender]:radio').on('click',function(e){
                        if($(this).val()==3){
                            $('#id_dni_type').val(22).change()
                            $('#company-info').show("slow")
                        }else{
                            $('#id_dni_type').val(0).change()
                            $('#company').val("")
