    ...
    {if $nb_products > $nArray[0]}
        <form action="{if !is_array($requestNb)}{$requestNb|escape:'html':'UTF-8'}{else}{$requestNb.requestUrl|escape:'html':'UTF-8'}{/if}" method="get" class="nbrItemPage">
            <div class="clearfix selector1">
                ...
                {/if}
                <label style="display: none;" for="nb_item{if isset($paginationId)}_{$paginationId}{/if}">
                    {l s='Show'}
                </label>
                {if is_array($requestNb)}
                    ...
                    {/foreach}
                {/if}
                <select name="n" id="nb_item{if isset($paginationId)}_{$paginationId}{/if}" class="form-control text-left">
                    <option value="100" {if $n eq '100'}selected="selected"{/if}>TAMAÑO</option>
                ...
                <span  style="display: none;">{l s='per page'}</span>
            </div>
        </form>
    {/if}
    <!-- /nbr product/page -->
