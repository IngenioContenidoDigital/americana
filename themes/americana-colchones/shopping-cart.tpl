...
<h1 id="cart_title" class="page-heading">{l s='Shopping-cart summary'}
    {if !isset($empty) && !$PS_CATALOG_MODE}
        <span style="display: none;" class="heading-counter">{l s='Your shopping cart contains:'}
...
{*                        <th class="cart_avail item text-center">{l s='Availability'}</th>*}
                    ...
                    <th class="cart_quantity item text-center">{l s='CANTIDAD'}</th>
                            ...
                            <td rowspan="{$rowspan_total}" colspan="2" id="cart_voucher" class="cart_voucher">
                                {if $voucherAllowed}
                                    <form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
                                        <fieldset>
                                            {*<h4>{l s='Vouchers'}</h4>*}
                                            <input type="text" class="discount_name form-control" id="discount_name" placeholder="Código de cupón" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
